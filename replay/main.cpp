...
DisplayHelp(void)
{
	...
	printf(
		...
		"  That includes the \"execute\" tool itself: an action's identity covers the expanded command line, not\n"
		"  the bytes of the binary or script it names, so rebuilding the tool alone does not re-run the action.\n"
		"  List the tool in \"inputs\" whenever a new build of it must invalidate what it produced.\n"
		...
		"  When the index is written it usually also records its own crc32c, and its inode, size and mtime\n"
		"  at that moment, in a \"public.replay.store-crc32c\" attribute - readable with \"xattr -px\", and\n"
		"  skipped silently where xattrs are unsupported. It says what the index contained when it was\n"
		"  published, not what it contains now: an attribute cannot notice bit rot, which moves neither\n"
		"  size nor mtime. It is deliberately not a public.fingerprint.* name, since those are trusted in\n"
		"  place of reading the file, and under one a corrupted index would report its pre-corruption hash\n"
		"  exactly when someone was investigating. What protects the index is its trailer checksum.\n"
