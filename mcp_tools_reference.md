...
**Literal vs regex:** default to **literal** matching (`isRegex` omitted/false) whenever you know the exact text to change — it is safer and idempotent (re-running won't double-apply; a target that's no longer present errors instead of mangling). Reach for `isRegex: true` only when the target is variable or you need a pattern across many lines.

Each edit item has:
- `oldText` (required) — text to find. Prefer a literal (standard mode) for known/specific text. **Standard mode** (no extended flags): tries exact match first, then falls back to whitespace-normalized line matching (strips common leading indent per block, then compares). At the default `limit: 1` a literal `oldText` must be **unique** — if it occurs more than once the edit is ambiguous and fails with `-32603` (`oldText is not unique: found N occurrences …`); add surrounding context so it matches one place, or raise `limit`. **Extended mode** (`isRegex: true`): ECMAScript (JavaScript) regex pattern, exempt from the uniqueness guard (`limit: 1` edits the first match). The pattern runs against the whole file content with multiline anchoring: `^` and `$` match at every line boundary (so `^(.*)$` matches the first line, or every line with `limit: 0`), and `.` does **not** cross newlines.
...
- `limit` — max replacements (default 1; 0 = unlimited). At `limit: 1` a literal `oldText` must be unique (see above); `limit: 0`/`N` opts into multiple matches. Extended: limit ≠ 1 disables whitespace-normalized fallback.
- `isRegex` — treat `oldText` as an ECMAScript regex pattern (default false). Extended. **Keep it false for known text** (literal is simpler and idempotent); use regex only for variable text / patterns across many lines. Add parentheses to capture text for reuse as `\1`–`\9` in `newText`.
